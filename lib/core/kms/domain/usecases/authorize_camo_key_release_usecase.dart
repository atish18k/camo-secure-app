// -----------------------------------------------------------------------------
// Imports
// -----------------------------------------------------------------------------
import '../../../shared/result/camo_result.dart';
import '../entities/camo_key_release_context.dart';
import '../entities/camo_key_release_decision.dart';
import '../repositories/camo_kms_repository.dart';

// -----------------------------------------------------------------------------
// Use Case
// -----------------------------------------------------------------------------
final class AuthorizeCamoKeyReleaseUseCase {
  const AuthorizeCamoKeyReleaseUseCase(this._repository);
  final CamoKmsRepository _repository;
  Future<CamoResult<CamoKeyReleaseDecision>> call(
    CamoKeyReleaseContext context,
  ) {
    return _repository.authorizeKeyRelease(context);
  }
}
